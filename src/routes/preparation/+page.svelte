<script lang="ts">
  ...
  const quizIntroItems = [
    ...
    {
      description: 'To make the flagging more transparent, semantic features are extracted from the post:',
	...
	onMount(() => {
		const saved = typeof localStorage !== 'undefined' ? localStorage.getItem('preparationStep') : null;
		if (saved !== null) {
...
<div class="min-h-screen bg-gray-100 py-12">
	...
	{:else if step === 5}
		...
		/>
	{:else if step === 6}
		<PreparationCompletion onContinue={handleContinueToExperiment} {quizResults} />
	{/if}
</div>
